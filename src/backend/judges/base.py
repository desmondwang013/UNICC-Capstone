...
class BaseJudge(ABC):
    ...
    async def evaluate(self, input: EvaluationInput) -> JudgeVerdict:
        """Wrapper: logs start/finish timing, then calls _evaluate()."""
        t0 = time.time()
        log.info(f"  → {self.JUDGE_NAME}: starting...")
        verdict = await self._evaluate(input)
        log.info(f"  ✓ {self.JUDGE_NAME}: done in {time.time()-t0:.1f}s (score={round(verdict.overall_score)}, tier={verdict.risk_tier.value})")
        return verdict
